...
> 平台状态：Windows 当前版本为 1.6.1，Android 当前版本为 1.6.4。Mac 暂时保留最后确认正常的 1.1.0 arm64 构建，不从当前 `main` 直接重发；详见 `MAC稳定版基线.md`。

## 当前功能

- 错题录入、答案展开、分类修改和带确认的删除。
- Windows 与手机录题页均提供快捷标题生成器：从可维护的书籍代号中选择一本，填写纯数字章节号和题号，可附加自由说明，生成如 `1000a-0-12(xxxx-xx-xx)` 的标题；最终标题仍可完全自定义。
...
- 手机裁剪器默认进入“移动照片”模式，可单指拖图直接选择保留部位；切换到“移动选框”后可拖动整个白框，四角可改变范围。两种模式均提供方向微调按钮，照片另有向左/向右旋转、加减缩放、滑杆和双指缩放，不依赖复杂手势也能完成裁剪。
