...
> 平台状态：Windows 当前版本为 1.6.1，Android 当前版本为 1.6.2。Mac 暂时保留最后确认正常的 1.1.0 arm64 构建，不从当前 `main` 直接重发；详见 `MAC稳定版基线.md`。
...
- 手机裁剪器默认进入“移动照片”模式，可单指拖图直接选择保留部位；切换到“移动选框”后可拖动整个白框，四角可改变范围。两种模式均提供方向微调按钮，照片另有加减缩放、滑杆和双指缩放，不依赖复杂手势也能完成裁剪。
